...
public class SecurityConfig { //Spring Security 5.7 이상에서 권장하는 방식
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(authz -> {
            //요청경로는 인증(로그인)된 사용자만 허용
            authz.requestMatchers("/imgboard/insert").authenticated()
                    .requestMatchers("/imgboard/detail").authenticated()
                    .requestMatchers("/imgboard/download").authenticated()
                   //요청경로는 ADMIN 권한이 있는 사용자만 허용
                    .requestMatchers("/imgboard/update").hasAuthority("ADMIN") //user.getRole()에서 반환한 값과 hasAuthority()가 매칭
                    .requestMatchers("/imgboard/del").hasAuthority("ADMIN")
