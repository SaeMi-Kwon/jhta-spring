...
public class SecurityConfig { //Spring Security 5.7 이상에서 권장하는 방식
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.formLogin(formLoginConfigurer -> {
            formLoginConfigurer
                    //.usernameParameter("id") username파라미터 이름이 (name=)id인 경우
                    //.passwordParameter("pwd") password파라미터 이름이 (name=)pwd인 경우
                    //GET 방식으로 로그인 화면을 보여주는 URL. 이건 우리가 직접 만드는 페이지
                    .loginPage("/loginPage") //로그인페이지 경로 지정
